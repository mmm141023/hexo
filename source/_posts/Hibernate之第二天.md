...
​	
	...
	2. Hibernate的持久化对象的状态
		* Hibernate为了管理持久化对象：将持久化对象分成了三个状态
			* 瞬时态:Transient  Object
				* 没有持久化标识OID, 没有被纳入到Session对象的管理，数据库中没有
			
			* 持久态:Persistent Object
				* 有持久化标识OID,已经被纳入到Session对象的管理，数据库中有
			
			* 脱管态（游离态）:Detached Object
				* 有持久化标识OID,没有被纳入到Session对象的管理，数据库中有
...
​	
	...
	2. 事务的特性
		* 原子性A	-- 事务不可分割.
		* 一致性C	-- 事务执行的前后数据的完整性保持一致.
		* 隔离性I	-- 一个事务执行的过程中,不应该受到其他的事务的干扰.
		* 持久性D	-- 事务一旦提交,数据就永久保持到数据库中.
